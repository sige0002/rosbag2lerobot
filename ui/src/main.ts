// Panel wiring for the bagel UI.
//
// One page, four stacked panels mirroring the convert loop:
//   1. Browse bag      -> inspect
//   2. Scaffold config -> validate-config
//   3. Convert         -> poll progress
//   4. Quality/preview -> validate-dataset / preview iframe
//
// Each panel displays the CLI `command` returned by its API call.

import * as api from "./api.js";
import type { BrowseEntry, Root } from "./api.js";
import {
  appendMessageList,
  byId,
  clear,
...
inspectBtn.addEventListener("click", () => {
  void withBusy(inspectBtn, async () => {
    ...
    try {
      ...
      clear(inspectStatus);
      showJson(inspectReport, res.report);
    } catch (e) {
      showError(inspectStatus, e);
    }
  });
...
scaffoldBtn.addEventListener("click", () => {
  void withBusy(scaffoldBtn, async () => {
    const bags = selectedBagList();
    if (bags.length === 0) {
      showStatus(validateConfigStatus, "warn", "Select at least one bag in Panel 1 first.");
      return;
    }
    showStatus(validateConfigStatus, "info", "Generating config...");
    try {
...
validateConfigBtn.addEventListener("click", () => {
  void withBusy(validateConfigBtn, async () => {
    ...
    if (configText.value.trim() === "") {
      showStatus(validateConfigStatus, "warn", "Config is empty. Generate or paste a config first.");
      return;
    }
...
async function pollJob(jobId: string): Promise<void> {
  try {
    ...
    if (st.state === "done") {
      ...
      // Make this output available to the quality/preview panel.
