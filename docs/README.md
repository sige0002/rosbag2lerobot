...
`bagel` のドキュメントは以下の 7 本に整理されています。用途から辿ってください。
...
| `inspect`, `validate-msg` サブコマンド | [cli_reference.md](cli_reference.md) |
| `validate-config`, `audit-timestamps` サブコマンド | [cli_reference.md](cli_reference.md) |
...
`src/bagel/configs/` 以下にサンプル YAML が置かれています。
